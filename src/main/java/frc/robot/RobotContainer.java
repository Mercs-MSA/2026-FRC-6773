...
public class RobotContainer {
  ...
  public PowerDistribution pdh = new PowerDistribution(4, ModuleType.kRev);
  ...
  private void configureButtonBindings() {
    ...
    controller.rightTrigger(0.5).whileTrue(teleopCommands.trackHub());
    controller.rightTrigger(0.5).whileFalse(teleopCommands.idleShooter());
  ...
  @AutoLogOutput(key = "CHECK_THIS_PLEASE")
  public double[] logCurrentDraws() {
    pdh.clearStickyFaults();
    double[] currents = pdh.getAllCurrents();

    // Drivetrain
    Logger.recordOutput("CurrentDraws/Overall", pdh.getVoltage());
    ...
    Logger.recordOutput("CurrentDraws/LeftBackRotation", currents[17]);
    Logger.recordOutput("CurrentDraws/RightBackDrive", currents[16]);
    Logger.recordOutput("CurrentDraws/RightBackRotation", currents[15]);
    Logger.recordOutput("CurrentDraws/RightFrontDrive", currents[14]);
    Logger.recordOutput("CurrentDraws/RightFrontRotation", currents[13]);

    // Mechanisms
    Logger.recordOutput("CurrentDraws/Spindexer", currents[0]);
    Logger.recordOutput("CurrentDraws/OmniWheelTransfer", currents[1]);
    Logger.recordOutput("CurrentDraws/Transfer", currents[2]);
    Logger.recordOutput("CurrentDraws/IntakePivot", currents[3]);
    Logger.recordOutput("CurrentDraws/IntakeMotor", currents[4]);
    Logger.recordOutput("CurrentDraws/Turret", currents[5]);
    Logger.recordOutput("CurrentDraws/Hood", currents[6]);
    Logger.recordOutput("CurrentDraws/ShooterRight", currents[7]);
    Logger.recordOutput("CurrentDraws/ShooterLeft", currents[8]);
